...
- Promo band [NEW] — built as `AppPromoBanner.vue`, the front page's "Tillsammans för en grönare framtid". A contained panel between the categories and the services: a wide photograph, a leaf mark in a ring, a headline, one sentence and a single CTA, all over a green scrim that runs flat under the text and fades to reveal the picture from `lg` up. No counterpart on the old site; built from a supplied design. Content is `app/data/promo.ts`, photograph `public/promo/`, its uncropped source and the derivation recipe `docs/assets/promo/`. Distinct from "Campaign / promo panel" below, which is an old-site block this does not replace.
