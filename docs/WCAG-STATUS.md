...
| `AppCategoryGrid` | ✅ pass | Ten tiles, one link each, named by the visible label; renders are `alt=""` so the category is not announced twice. `ul`/`li`, so the count is announced before the set is walked. Section named by its own `h2` via `aria-labelledby`. The tile is a gradient, so both stops were computed: label `ink` is **11.25:1** at worst (on `surface-sunken`, 12.06:1 on `surface-raised`), hover `brand-deep` **9.66:1** at worst. The focus ring is the default `brand`, drawn outside the tile on the section's white — 4.40:1 there, 3.86:1 against the tile edge it touches, so it clears 1.4.11 on both sides. Nothing clips it: the block deliberately has no `overflow-hidden`. Tiles are 112px tall, far past 2.5.8. |
| `AppServiceSlider` | ✅ pass | **Re-audited in-browser after the teal recolour and the height changes** — every figure below is computed from the rendered page, not from the tokens. Contrast on the white card: `h2` **12.82:1**, title **12.82:1**, description `ink-muted` **5.74:1**, eyebrow and "Läs mer" `support` **5.50:1** (10px and 14px, so the 4.5:1 bar, not 3:1), arrow icons **12.82:1**. "Läs mer" hovers to an underline, not a second colour — there is one `support` token, and the state does not rest on colour (1.4.1). **2.5.8**: the link is stretched over the whole card by `after:inset-0`; hit-tested with `elementFromPoint` at five points — corners, centre, over the image and over the title — all return the anchor, so the target is 268×316, and the arrows are 42×42. **2.4.7**: ring is `brand` 2px at 2px offset, **4.40:1** on the white card, and its box sits inside the track bounds so the `overflow-x` container does not clip it. The gap above the link was moved from padding on the link to `mb-4` on the paragraph, because padding inflated the anchor box upward and the ring landed across the description's last line. **1.4.12**: text-spacing overrides applied live (line-height 1.5, letter-spacing .12em, word-spacing .16em) — no `line-clamp` truncation before or after, so no content loss. **1.4.10 / 1.4.4**: no horizontal page scroll at 320px, 400px or a 750px (≈200%) viewport; the track itself scrolls horizontally by design, which 1.4.10 permits for the content it exempts. Names and structure: `ul`/`li`, section labelled by its own `h2`, six `h3`s, all images `alt=""`, arrow controls `aria-hidden` with `tabindex="-1"` — everything they reach is reachable by tabbing card to card, which scrolls the track natively, so 2.1.1 holds without two extra tab stops. Link names are "Läs mer" + the card heading, so no two are alike. Nothing auto-moves (2.2.2 n/a); the reduced-motion check is in the click handler because an explicit `behavior` in `scrollBy` overrides a CSS `motion-reduce:scroll-auto`. **Known cost, not a failure**: the stretched link means the title and description cannot be selected with the mouse. **Not verified with a screen reader.** |
| `AppPromoBanner` | ✅ pass | New block, **measured in-browser** across 320 / 360 / 390 / 500 / 767 / 768 / 1023 / 1024 / 1280 / 1440 / 1580 / 1920 / 2560px, not derived from the tokens. A contained panel — `max-w-site px-gutter`, `rounded-md` — carrying white text on a photograph, so like `AppHeroSlide` it has no knowable contrast without its scrim; the scrim here is **unconditional**, no data can turn it off, so A11Y-10's failure mode does not exist in this block. **Re-audited three times** — when the band was lightened, when the palette was warmed to a 94° yellow-green, and when it stopped being full-bleed; every figure below is from the current build. Scrim (`eco-deep` `#2f561f` at 80%) composites to `#59784c` over the brightest frame possible, white on that **4.98:1**; over the darkest, 10.78:1. That margin has narrowed at each pass — 7.98 → 5.29 → 4.98 against a 4.5 floor — because brightness and warmth are both bought with luminance; it is the first number to recompute if the band is restyled again. **The stop positions are the other load-bearing part, and they are percentages of the band, not of the viewport** — which is why containing the block invalidated the previous set and forced a re-measure. The rule they must obey is that the scrim is still flat where the text ends; the first full-bleed version broke it, holding flat only to 48% while the text ran to 62.5% at 768px, putting the last words on ~44% scrim (**~2.4:1** over a bright frame). Now: the reveal starts at `lg`, and below it both gradient stops are the same opaque colour, so the fill is flat and text position cannot matter (the text takes 70.4% of the band at 768px, which no fade could clear). At and above `lg` the flat region runs to 58% against a measured worst case of **52.5% at exactly 1024px**, after which the share falls monotonically — 36.3% at 1440, pinned at 34.4% from 1580px up, where the band stops growing at `max-w-site` minus two gutters. Button label white on `eco` **4.82:1**, hover step **5.57:1**. **1.4.11 on the pill was closed rather than accepted here**: warming took the fill to **1.03:1** against the scrim — equal luminance, separated only by hue — so the pill carries a `white/70` ring, 3.33:1 against the scrim and 3.23:1 against the fill. A deliberate divergence from `AppHeroSlide`'s overlay button, which still leans on its label alone. Icon ring same treatment, **3.33:1**. **2.4.7 verified, not derived**: `--color-focus` is flipped to white as `.on-ink` does, because brand pink on this band is 2.92:1. The link was focused programmatically and `:focus-visible` confirmed to match, then screenshotted — the ring draws white around the pill and is not clipped by the panel's `overflow-hidden`, which it could have been: the indicator is a 2px outline at 2px offset, so 4px beyond the link's box, against 36px of clearance to the panel's left edge and 32px to its bottom. It also clears the paragraph's last line, which is what `mt-1` on the link rather than padding buys. Link target **181×40**, past 2.5.8, four pixels short of the 2.5.5 AAA bar like every other button here. Section named by its own `h2`; the photograph is `alt=""` by type, and the leaf is `aria-hidden` because the headline beside it says the same thing in words. **1.4.10**: `documentElement.scrollWidth` never exceeded the viewport at any of the thirteen widths. `min-h-50` is a floor, not a height — the band draws at 220px from 767px up and grows on its own to 271px at 320px, where the paragraph takes more lines. **1.4.12 applied live** (line-height 1.5, letter-spacing .12em, word-spacing .16em, paragraph margin 2em): the band grew 220px → 267px with no overlap between heading, paragraph and button, no content past the panel's bottom edge, and no horizontal scroll — so the floor lets a long translation or a user's own spacing push it taller instead of clipping (1.4.4, 1.4.12). **Not verified with a screen reader.** |
